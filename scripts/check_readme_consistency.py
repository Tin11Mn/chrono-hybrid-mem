...
def validate_readmes(root: Path = ROOT) -> list[str]:
    ...
    english_without_fences = FENCED_CODE.sub("", english)
    expected_code_blocks = FENCED_CODE.findall(english)
    expected_inline_code = Counter(INLINE_CODE.findall(english_without_fences))
    expected_numbers = Counter(NUMBER_TOKEN.findall(english_without_fences))
    language_files = set(README_LABELS)
    expected_content_links = sorted(
        target
        for target in _local_targets(english)
    ...
    for name, text in texts.items():
        ...
        if FENCED_CODE.findall(text) != expected_code_blocks:
            errors.append(f"{name}: fenced code blocks differ from README.md")
        if Counter(INLINE_CODE.findall(text_without_fences)) != expected_inline_code:
            errors.append(f"{name}: inline code tokens differ from README.md")
        if Counter(NUMBER_TOKEN.findall(text_without_fences)) != expected_numbers:
            errors.append(f"{name}: numeric tokens differ from README.md")
        ...
        if content_links != expected_content_links:
            errors.append(f"{name}: content link targets differ from README.md")

...
## 현재 안정 파이프라인: P4-A q2 + bm25 selection + Session-Fact 의미론적 레이어(현재 최고)
...
그 위에 **Session-Fact 의미론적 레이어(SF v2)를 연구상 현재 최고로 채택**했습니다. 기본적으로 비활성화되어 있고(`session_fact_layer=False`; 오프라인 생성 세션별 사실 캐시와 명시적 활성화 필요), Add/Search API와 안정 기본값을 바꾸지 않으며, 전체 1,976문항 Hit@1을 0.5850에서 0.6108로 끌어올립니다(paired bootstrap p=1.000; 섹션 B 참조).
...
저장소에는 LoCoMo에서 실행한 전체 제출 후 로컬 실행 결과가 기록되어 있습니다(로컬 Qwen3-4B 프록시이며 공식 leaderboard 결과가 아닙니다). **제출 후 연구 진행 경과**(단계마다 범위가 다르므로 범위가 다른 행을 직접 비교하지 마십시오):

| 단계 | 방법 | Hit@1 | Hit@3 | Hit@10 | MRR | 범위 |
|---|---|---:|---:|---:|---:|---|
| 초기 연구 | [v0.2.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/v0.2.0) 공식 동결 검색 | 0.2671 | 0.4355 | 0.5695 | 0.3567 | 1,977개 질의 |
| 초기 연구 | [research-v0.3.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.3.0) 로컬 하이브리드 검색 | 0.4355 | 0.6186 | 0.7577 | 0.5183 | 1,977개 질의 |
| 초기 연구 | [research-v0.4.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.4.0) Qwen 재순위 + 시간 키 | 0.5225 | 0.6808 | 0.7653 | 0.5856 | 1,977개 질의 |
| 현재 계열 | P1 structured planner(기준선) | 0.5779 | 0.7176 | 0.7601 | 0.6497 | 1,976개 질의 |
| 현재 계열 | P4-A q2(ablation) | 0.5779 | 0.7201 | 0.7642 | 0.6504 | 1,976개 질의 |
| 현재 계열 | P4-A q2 + bm25 selection | 0.5850 | 0.7323 | 0.7809 | 0.6618 | 1,976개 질의 |
| 현재 계열 | **+ Session-Fact 의미론적 레이어(현재 최고)** | **0.6108** | **0.7677** | **0.8219** | **0.6929** | 1,976개 질의 |

> 범위 주석: 1,977개 질의 행은 offset 758 중단이 확인되기 이전의 것이며 당시 완료 가능했던 전체 질의를 다룹니다. 이후 모든 방법이 offset 758을 동일하게 제외하므로 현재 계열은 1,976개 질의입니다(초기 행과 약 한 개 질의 차이. 같은 계열·같은 범위 안에서만 비교하십시오). 초기 행은 각 버전의 README에서 가져온 것이며, 현재 계열의 동일 범위 비교와 유의성은 아래에 있습니다.

**현재 계열 동일 범위 비교**(offset 758은 모든 방법에서 동일하게 제외되는데, 지나치게 긴 해당 대화가 추론 서버를 확실히 중단시키기 때문입니다):
...
## P3 증거 그래프 실험(LoCoMo 로컬 프록시)

P3는 새 내용을 생성하지 않고 원본 메시지 위에 추적 가능한 보조 증거 구조를 구축합니다. 모든 노드와 엣지는 원본 메시지를 가리키며, `/search`는 계속 원본 증거만 반환합니다. P3는 기본적으로 꺼져 있습니다. 아래 결과는 Qwen3-4B를 로컬 프록시로 사용한 고정 20문항 LoCoMo 개발 슬라이스에만 적용되며, 공식 leaderboard 전체로 일반화할 수 없습니다.

| 하위 실험 | 방법 및 결과 | 현재 결론 |
|---|---|---|
| P3-A 엄격한 관계 그래프 | 원본 메시지 419개 중 독립적으로 입증된 관계 엣지는 3개뿐(소스 커버리지 0.72%). | 추출 커버리지가 너무 낮아 paired retrieval 평가로 진행하지 않았다. |
| P3-B1 소스-로컬 엔터티 앵커 | 정확한 엔터티 언급은 363/419개 소스 메시지를 포괄(86.63%); Hit@1 0.40 → 0.35, Hit@3 0.45 → 0.45, Hit@10 0.55 → 0.50, MRR 0.45 → 0.41. | 이 슬라이스에서 성능이 저하되어 전역 기본값이 아니다. |
| P1.1 인접 원본 메시지 확장 | Hit@1/3/10/MRR가 baseline과 동일: 0.40/0.45/0.55/0.45. | 개선 없음; 더 큰 평가로 승격하지 않는다. |
| P3-C 명시적 시간 상태 그래프 | 독립적인 ablation이 아직 구현되지 않았다. | 미검증이며 시간 데이터에 대한 결론은 없다. |

## P2 집합 인식 재정렬 실험(기각)

P2는 구조화된 계획의 증거 필요 토큰을 이용해 P1 모델 순위 뒤에 기존 원본 후보를 재정렬합니다. 기본적으로 꺼져 있고 모델 호출을 추가하지 않습니다. 고정 20문항 로컬 LoCoMo 프록시에서 Hit@1은 `0.40`으로 같고 Hit@3와 MRR은 개선되었지만, 계획을 고정한 공개 합성 계층화 35개 사례에서는 Hit@1이 `1.00`에서 `0.8571`로, MRR이 `1.00`에서 `0.9286`으로 하락했으며 금지된 증거가 최상위가 되는 비율도 증가했습니다. P2는 재현 가능한 실험 및 실패 분석으로만 보존하며 기본 경로나 공식 제출 주장에는 사용하지 않습니다.

이 실험은 LoCoMo에서 그래프나 인접 확장을 기본 활성화한다는 가설을 기각한 것이지, 그래프 방법 일반이 효과 없다는 뜻은 아닙니다. 향후 활성화는 입력에서 관찰 가능한 관계, 다단계, 시간 신호에만 기반하며 비공개 벤치마크 식별자나 정답에는 적응하지 않습니다.

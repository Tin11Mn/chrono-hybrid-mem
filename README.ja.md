...
## 現在の安定版パイプライン：P4-A q2 + bm25 selection + Session-Fact セマンティックレイヤー（現在最良）
...
その上で、**Session-Fact セマンティックレイヤー（SF v2）を研究上の現在最良として採用**します。既定では無効（`session_fact_layer=False`。オフライン生成のセッション別事実キャッシュと明示的な有効化が必要）で、Add/Search API や安定した既定値を変更せず、全 1,976 問の Hit@1 を 0.5850 から 0.6108 に引き上げます（paired bootstrap p=1.000。セクション B 参照）。
...
このリポジトリには、LoCoMo（ローカルの Qwen3-4B プロキシ。公式リーダーボードの結果ではない）での提出後ローカル実行の全結果が記録されています。**提出後研究の進展**（各段階でスコープが異なるため、スコープの異なる行を直接比較しないでください）:

| 段階 | 手法 | Hit@1 | Hit@3 | Hit@10 | MRR | スコープ |
|---|---|---:|---:|---:|---:|---|
| 初期研究 | [v0.2.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/v0.2.0) 公式凍結検索 | 0.2671 | 0.4355 | 0.5695 | 0.3567 | 1,977 クエリ |
| 初期研究 | [research-v0.3.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.3.0) ローカルハイブリッド検索 | 0.4355 | 0.6186 | 0.7577 | 0.5183 | 1,977 クエリ |
| 初期研究 | [research-v0.4.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.4.0) Qwen 再ランク + 時間キー | 0.5225 | 0.6808 | 0.7653 | 0.5856 | 1,977 クエリ |
| 現在の系統 | P1 structured planner（ベースライン） | 0.5779 | 0.7176 | 0.7601 | 0.6497 | 1,976 クエリ |
| 現在の系統 | P4-A q2（アブレーション） | 0.5779 | 0.7201 | 0.7642 | 0.6504 | 1,976 クエリ |
| 現在の系統 | P4-A q2 + bm25 selection | 0.5850 | 0.7323 | 0.7809 | 0.6618 | 1,976 クエリ |
| 現在の系統 | **+ Session-Fact セマンティックレイヤー（現在最良）** | **0.6108** | **0.7677** | **0.8219** | **0.6929** | 1,976 クエリ |

> スコープの注記：1,977 クエリの行はオフセット 758 のハングが特定される前のもので、当時実行可能だった全クエリを対象とします。それ以降は全手法がオフセット 758 を一様に除外するため、現在の系統は 1,976 クエリです（初期の行とは約一件分異なります。同じ系統・同じスコープ内でのみ比較してください）。初期の行は各バージョンの README に由来します。現在の系統の同一スコープ比較と有意性は以下に示します。

**現在の系統の同一スコープ比較**（オフセット 758 は、その過度に長い会話が推論サーバーを確実にハングアップさせるため、すべての手法で一様に除外）:

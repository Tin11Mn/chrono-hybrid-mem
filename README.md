...
仓库记录了以下在 LoCoMo 本地切片上完成的赛后全量研究结果（全部为本地
Qwen3-4B 代理口径，非官方排行榜结果）：

**当前主方法（P4-A evidence-need 独立检索 + need 候选 bm25 选取，
1976 个可完成查询）**
...
| **P4-A q2 + bm25 选取（主方法）** | **0.5850** | **0.7323** | **0.7809** | **0.6618** | **0.6086** |

* Evidence Recall@10 为各自证据池口径（差约 6 条源自 offset 758 统一排除）；
同口径逐条重算待补。nDCG@10（NEW）0.6914；Recall@5（NEW）0.7606。
NEW vs P1 的 paired bootstrap（10,000 次）95% CI：Hit@1 [-0.0051, +0.0197]
含 0（未达统计显著）、MRR [+0.0036, +0.0207]、Hit@10 [+0.0116, +0.0299]
（均显著）。offset 758（超长对话必然挂起）对全部方法一致排除，故可完成
查询为 1976 而非 1977。

评测协议、逐题明细与复现见：
[全量评测（1976）](docs/EVALUATION_NEW_METHOD_1976.md)、
[论文结果](docs/CHRONOHYBRIDMEM_RESULTS_FOR_PAPER.md)、
[复现指南](docs/CHRONOHYBRIDMEM_REPRO_NEW_METHOD.md)、
[失败分析](docs/CHRONOHYBRIDMEM_FAILURE_ANALYSIS.md)。

**历史记录**：2026-08-16 的 P1 全量运行（1977 问，Hit@1 0.5761 /
Hit@3 0.7157 / Hit@10 0.7618 / MRR 0.6479）仍可在
[P1 本地模型评测](docs/P1_LOCAL_EVALUATION.md) 中查阅；该次运行是更早的
历史全量结果，未启用 evidence-need 通道，且当时未跑同题集的扁平规划器
对照，不能作为扁平规划器到 P1 的增益证据。
...
| [`research/p3-evidence-graph`](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research/p3-evidence-graph) | 当前主方法：P4-A evidence-need 检索 + bm25 选取（1976 问全量评测 Hit@1 0.5850） | 活跃研究分支 |

...
默认分支 `main` 保留经过验证的稳定版赛后本地研究路径 P1。P3 Evidence Graph 代码仅作为默认关闭的实验性研究能力保留，不属于稳定检索路径。
...
## 当前稳定版流水线：P4-A q2 + bm25 选取 + Session-Fact 语义层（当前最佳）
...
在此基础上，**Session-Fact 语义层（SF v2）已采纳为研究当前最佳**：它默认关闭
（`session_fact_layer=False`，需离线生成 per-session 事实缓存后显式开启），
不改变 Add/Search API 与稳定默认配置，但把全量 1,976 问 Hit@1 从 0.5850 提升到
0.6108（paired bootstrap p=1.000，见 B 节）。
...
排行榜结果）。**赛后研究演进**（各阶段口径不同，不可跨段直接对比）：

| 阶段 | 方法 | Hit@1 | Hit@3 | Hit@10 | MRR | 口径 |
|---|---|---:|---:|---:|---:|---|
| 早期研究 | [v0.2.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/v0.2.0) 官方冻结检索 | 0.2671 | 0.4355 | 0.5695 | 0.3567 | 1,977 问 |
| 早期研究 | [research-v0.3.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.3.0) 本地混合检索 | 0.4355 | 0.6186 | 0.7577 | 0.5183 | 1,977 问 |
| 早期研究 | [research-v0.4.0](https://github.com/Tin11Mn/chrono-hybrid-mem/tree/research-v0.4.0) Qwen 重排 + 时间键 | 0.5225 | 0.6808 | 0.7653 | 0.5856 | 1,977 问 |
| 当前线 | P1 结构化规划器（基线） | 0.5779 | 0.7176 | 0.7601 | 0.6497 | 1,976 问 |
| 当前线 | P4-A q2（消融） | 0.5779 | 0.7201 | 0.7642 | 0.6504 | 1,976 问 |
| 当前线 | P4-A q2 + bm25 选取 | 0.5850 | 0.7323 | 0.7809 | 0.6618 | 1,976 问 |
| 当前线 | **+ Session-Fact 语义层（当前最佳）** | **0.6108** | **0.7677** | **0.8219** | **0.6929** | 1,976 问 |

> 口径说明：1,977 问为 offset 758 挂起问题识别前的全量可完成查询；识别后
> 所有方法统一排除 758，当前线均为 1,976 问（含早期行的 758 挂起差异约
> 1 条，同线同口径方可对比）。早期行来源为各版本 README；当前线详细同口径
> 对比与显著性见下表。

**当前线同口径对比**（offset 758 对所有方法一致排除——该超长对话必然挂起
推理服务器）：
...
P2 在 P1 模型排序之后尝试依据结构化计划中的证据需求词，重排既有原始候选；默认关闭，且不增加模型调用。固定 20 题 LoCoMo 本地代理中 Hit@1 持平（`0.40`），Hit@3 与 MRR 上升；但在冻结计划的 35 个公开合成分层案例中，Hit@1 从 `1.00` 降至 `0.8571`，MRR 从 `1.00` 降至 `0.9286`，并增加错误证据首位率。因此 P2 仅保留为可复现实验和失败分析，不得作为默认路径或官方提交结论。
...
- 在 P3 通过已声明的升级门槛前，它在 `main` 中仍是默认关闭的实验性研究能力；评测期间稳定版 P1 行为必须保持不变。
